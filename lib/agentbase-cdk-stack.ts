...
export interface AgentBaseStackProps {
  vpc: IVpc;
  config: StackConfig;
  extraValues?: object;
}

export class AgentBaseStackConstruct {
  id: string;
  scope: Construct;
  agentbaseProps: AgentBaseStackProps;
  props?: cdk.StackProps;
  vpc: IVpc;

  constructor(scope: Construct, id: string, agentbaseProps: AgentBaseStackProps, props: cdk.StackProps) {
    this.scope = scope;
    this.id = id;
    this.agentbaseProps = agentbaseProps;
    this.props = props;
    this.vpc = agentbaseProps.vpc;
  }
  build() {
    const scope = this.scope;
    const id = this.id;
    const agentbaseProps = this.agentbaseProps;
    const config = agentbaseProps.config;
    ...
    const appNodeSubnets: ec2.SubnetSelection = {
      subnets: config.cluster.managedNodeGroups.app.workerNodeSubnetIds && config.cluster.managedNodeGroups.app.workerNodeSubnetIds.length > 0
        ...
        : agentbaseProps.vpc.selectSubnets({ subnetType: SubnetType.PRIVATE_WITH_EGRESS }).subnets
    }
    ...
    const blueprintBuilder = blueprints.EksBlueprint.builder()
      .version(config.cluster.version)
      .addOns(...addOns)
      .resourceProvider(blueprints.GlobalResources.Vpc, new blueprints.DirectVpcProvider(agentbaseProps.vpc))
      .resourceProvider("db", new PostgresSQLResourceProvider({ vpc: agentbaseProps.vpc, config: config }))
      .resourceProvider("redis", new RedisResourceProvider({ vpc: agentbaseProps.vpc, config: config }))
    ...
    cdk.Tags.of(blueprint).add('marketplace', 'agentbase');
